...
public class BattleGroundManager {
    ...
    public static boolean registerPlayer(Player player, int tplId) {
        ...
        if (player.isInGroup2()) {
            if (player.getPlayerGroup2().getLeader().getObjectId() != player.getObjectId()) {
                PacketSendUtility.sendMessage(player, "Solo el líder del grupo puede registrar un grupo.");
                return false;
            }
                ...
                if (player.getPlayerGroup2().size() <= (template.getNbPlayers() - elyosWaitList.get(tplId).size())) {
                    if (highestLevel <= template.getJoinConditions().getMaxLevel() && lowestLevel >= template.getJoinConditions().getRequiredLevel()
                        ...
                        for (int i = 0; i < members.size(); i++) {
                            ...
                            PacketSendUtility.sendMessage(member[i], "Ahora estás registrado para el campo de batalla: " + template.getName());
                            PacketSendUtility.sendMessage(member[i], "Por favor espere mientras el equipo está en formación...");
                        }
                        computeReadiness(tplId);
                        return true;
                    }
                    PacketSendUtility.sendMessage(player, "Todos los miembros no pueden unirse a este campo de batalla.");
                    return false;
                }
                PacketSendUtility.sendMessage(player, "No estas listo para registrarte en este campo de batalla.");
                return false;
            }
        ...
        if (!player.isInGroup2()) {
            if ((player.getCommonData().getRace() == Race.ELYOS && !elyosWaitList.get(tplId).contains(player))
                ...
                PacketSendUtility.sendMessage(player, "Ahora estás registrado para el campo de batalla: " + template.getName());
                PacketSendUtility.sendMessage(player, "Por favor espere mientras el equipo está en formación...");
                computeReadiness(tplId);
                return true;
            }
            return false;
    ...
    public static void sendRegistrationForm(Player player) {
        ...
        // Registration form
        String html = HTMLService.HTMLTemplate("Registrarse en los campos de batalla", "Puedes registrarte para los siguientes campos de batalla :", acceptedLocations, 0, 0);
