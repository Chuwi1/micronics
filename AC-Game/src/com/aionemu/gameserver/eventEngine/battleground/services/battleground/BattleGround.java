...
public abstract class BattleGround {
    ...
    public void increasePoints(Player player, int value) {
        PacketSendUtility.sendMessage(player, "Has ganado " + value + " BG points.");
    ...
    public void decreasePoints(Player player, int value) {
        PacketSendUtility.sendMessage(player, "Has perdido " + value + " BG points.");
    ...
    public void invitePlayer(final Player player) {
        PacketSendUtility.sendPacket(player, new SM_MESSAGE(0, null, "El campo de batalla: " + template.getName() + " ahora está listo para comenzar. Serás teletransportado en 30 segundos. Que te diviertas :)", ChatType.BRIGHT_YELLOW_CENTER));
    ...
    public void start() {
        ...
        ThreadPoolManager.getInstance().schedule(new Runnable() {
            @Override
            public void run() {
                broadcastToBattleGround(template.getWaitTime() + " segundos antes de comenzar...", null);
                for (Player p : players) {
                    // reset stats
                    ...
                    if (p.battlegroundObserve == 1) {
                        ...
                        PacketSendUtility.sendMessage(p, "Ahora eres invisible");
                        p.setInvul(true);
                        PacketSendUtility.sendMessage(p, "Eres ahora inmortal.");
                    }
                }
            }
        }, 31 * 1000);
        ...
        ThreadPoolManager.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                openDoors();
                broadcastToBattleGround("El campo de batalla está abierto!", null);
                running = true;
            }
        ...
        ThreadPoolManager.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                if (running == true) {
                    broadcastToBattleGround("El campo de batalla terminara en 30 segundos!", null);
                }
            }
            ...
            public void run() {
                for (Player p : players) {
                    if (p.battlegroundObserve > 0) {
                        ...
                        PacketSendUtility.sendMessage(p, "El tiempo de la apuesta ha terminado.");
                    }
                }
            }
    ...
    public void end() {
        running = false;
        broadcastToBattleGround("¡La batalla ha terminado! Haga clic en el botón inferior derecho para mostrar el tablero de clasificación. Si estás muerto, solo usa el hechizo Return.", null);
        for (Player p : players) {
            if (p.battlegroundObserve > 0) {
                p.unsetVisualState(CreatureVisualState.HIDE20);
                PacketSendUtility.broadcastPacket(p, new SM_PLAYER_STATE(p), true);
                PacketSendUtility.sendMessage(p, "Ahora eres visible");
                p.setInvul(false);
                PacketSendUtility.sendMessage(p, "Eres ahora mortal.");
            }
        }
    }
            ...
            public void run() {
                for (final Player p : players) {
                    ...
                    if (p.getWorldId() == 110010000 || p.getWorldId() == 120010000) {
                        String message = "Quieres volver a jugar en un campo de batalla?";
                        RequestResponseHandler responseHandler = new RequestResponseHandler(p) {
                            public void acceptRequest(Creature requester, Player responder) {
                                if (p.getBattleGround() != null) {
                                    PacketSendUtility.sendMessage(p, "Ya estas registrado en un campo de batalla.");
                                    PacketSendUtility.sendMessage(p, "Usa tu hechizo de Retorno para salir del campo de batalla.");
                                    return;
                                } else if (p.battlegroundWaiting) {
                                    PacketSendUtility.sendMessage(p, "Ya estas registrado en un campo de batalla.");
                                    PacketSendUtility.sendMessage(p, "Use el comando .bg unregister para cancelar su registro.");
                                    return;
                                } else {
