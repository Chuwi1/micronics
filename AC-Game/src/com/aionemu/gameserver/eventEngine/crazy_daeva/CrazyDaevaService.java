            ...
            public void visit(final Player player) {
                ...
                if (player.getRndCrazy() >= 50 && player.getLevel() >= 65) {
                    ...
                    if (crazyCount == 1) {
                        ...
                        PacketSendUtility.event("Un objetivo ha sido elegido, Nombre: " + player.getName() + " ! Tienes 15 minutos para encontrarlo y matarlo (puedes matarlo tantas veces como desees durante este tiempo). QUE TE DIVIERTAS :)");
    ...
    private void sendEndSpreeMessage(Player victim, Creature killer, boolean isPvPDeath) {
        if (killer instanceof Player) {
            if (killer.getRace() == Race.ELYOS && victim.getRace() == Race.ASMODIANS) {
                ...
                PacketSendUtility.event("Objetivo " + victim.getName() + " fue asesinado por " + spreeEnder + "!");
    ...
    public void clearCrazy() {
        ThreadPoolManager.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                ...
                PacketSendUtility.event("Todo el Evento de Daeva terminó!");
