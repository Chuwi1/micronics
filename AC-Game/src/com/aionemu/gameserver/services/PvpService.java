      ...
      //Crazy kill
        if (EventSystem.ENABLE_CRAZY) {
            if (winner.getRndCrazy() >= EventSystem.CRAZY_LOWEST_RND) {
                ...
                AbyssPointsService.addGp(winner, 5);
            }
        }
        
        if (winner.getDotaLevel()== 0) {
        	winner.setDotaLevel(1); //Mata y subimos a 1
        	//Global Announce, String Custom
            World.getInstance().doOnAllPlayers(new Visitor<Player>() {
    			@Override
    			public void visit(Player player) {
    				PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1403137,winner.getName(),winner.getRace().getRaceDescriptionId(),victim.getName(),victim.getRace().getRaceDescriptionId()));
    			}
    		});
        	ThreadPoolManager.getInstance().schedule(new Runnable() {
				@Override
				public void run() {
					winner.setDotaLevel(0);
				}
			}, 10 * 1000); //Tiene 10 segundos para matar otra vez
        } else if (winner.getDotaLevel()== 1) {
        	winner.setDotaLevel(2); //Mata por segunda vez como doble kill
        	World.getInstance().doOnAllPlayers(new Visitor<Player>() {

        		@Override
        		public void visit(Player findedPlayer) {
        			PacketSendUtility.sendPacket(findedPlayer, new SM_SYSTEM_MESSAGE(1501208,winner.getName(),winner.getRace().getRaceDescriptionId(),victim.getName(),victim.getRace().getRaceDescriptionId()));
        		}

        	});
        } else if (winner.getDotaLevel()== 2) {
        	winner.setDotaLevel(3);//Mata por tercera vez como tripe kill
        	World.getInstance().doOnAllPlayers(new Visitor<Player>() {

        		@Override
        		public void visit(Player findedPlayer) {
        			PacketSendUtility.sendPacket(findedPlayer, new SM_SYSTEM_MESSAGE(1501209,winner.getName(),winner.getRace().getRaceDescriptionId(),victim.getName(),victim.getRace().getRaceDescriptionId()));
        		}

        	});
        } else if (winner.getDotaLevel()== 3) {
        	winner.setDotaLevel(4);//Mata por cuarta vez como tripe kill
        	World.getInstance().doOnAllPlayers(new Visitor<Player>() {

        		@Override
        		public void visit(Player findedPlayer) {
        			PacketSendUtility.sendPacket(findedPlayer, new SM_SYSTEM_MESSAGE(1501210,winner.getName(),winner.getRace().getRaceDescriptionId(),victim.getName(),victim.getRace().getRaceDescriptionId()));
        		}

        	});
        } else if (winner.getDotaLevel()== 4) {
        	winner.setDotaLevel(5);//Mata por quinta vez como tripe kill
        	World.getInstance().doOnAllPlayers(new Visitor<Player>() {

        		@Override
        		public void visit(Player findedPlayer) {
        			PacketSendUtility.sendPacket(findedPlayer, new SM_SYSTEM_MESSAGE(1501211,winner.getName(),winner.getRace().getRaceDescriptionId(),victim.getName(),victim.getRace().getRaceDescriptionId()));
        		}
