        ...
        getSiegeLocation().doOnAllPlayers(new Visitor<Player>() {
            ...
            public void visit(Player player) {
                if (player.getRace().name() == race.name()) {//dont know if this works...
                	if (getSiegeLocationId() == 1131 || getSiegeLocationId() == 1132  || getSiegeLocationId() == 1141  || getSiegeLocationId() == 1211 || getSiegeLocationId() == 1251) {
            ...
            for (int rewardedPC = 0; i < topPlayersIds.size() && rewardedPC < topGrade.getTop(); ++i) {
                ...
                if (LoggingConfig.LOG_SIEGE) {
                    ...
                    if (getSiegeLocation().getRace() == SiegeRace.ELYOS){
                    	log.info("SIEGE LA MISMA RAZA ELYOS NO GANARA GP ADICIONAL");
                    } else {
                    	log.info("SIEGE LA RAZA QUE PERDIO ASMODIAN GANA GP ADICIONAL");
                    }
                }
                ...
                if (getSiegeLocationId() == 1131 || getSiegeLocationId() == 1132  || getSiegeLocationId() == 1141  || getSiegeLocationId() == 1211 || getSiegeLocationId() == 1251) {
                	log.info("[SIEGE]  > [FORTRESS:" + getSiegeLocationId() + "] [RACE: " + getSiegeLocation().getRace() + "], RECIBE EL GP MINIMO POR LOWER FORTRESS");
                	switch (level) {//ap reward for fotress occupation
                        case HERO_DECORATION:
                            AbyssPointsService.addGp(pcd.getPlayer(), 50);
                            break;
                        case MEDAL:
                            AbyssPointsService.addGp(pcd.getPlayer(), 40);
                            break;
                        case ELITE_SOLDIER:
                            AbyssPointsService.addGp(pcd.getPlayer(), 30);
                            break;
                        case VETERAN_SOLDIER:
                            AbyssPointsService.addGp(pcd.getPlayer(), 20);
                            break;
                        default:
                            AbyssPointsService.addGp(pcd.getPlayer(), 10);
                            break;
                    }
                } else {
                	log.info("[SIEGE]  > [FORTRESS:" + getSiegeLocationId() + "] [RACE: " + getSiegeLocation().getRace() + "], RECIBE EL GP NORMAL POR FORTRESS");
